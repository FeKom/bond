...
public class ImpClientRepository implements ClientRepository {
	...
	//isso aq aplica o filtro where
		Predicate[] filters = conditions(ClientQuery.builder()
				.ids(Set.of(id))
				.build(), cb, root);
		if (filters.length > 0) {
	...
	private Predicate[] conditions(ClientQuery query,
			CriteriaBuilder cb,
		...
		// 2. FILTRO POR ID (Valor Único)
		// Se um único ID for fornecido, usa-se o predicado EQUAL.
		query.ids().ifPresent(id -> predicates.add(cb.equal(root.get("id"), id)));
